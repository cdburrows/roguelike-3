...
public class DungeonGenerator implements IDungeonGenerator {
    ...
    /**
     * Creates a DungeonGenerator with a given random seed and places specific
     * constraints on {@link IDungeon}s it generates.
     * 
    ...
    protected class KeyLevelRoomMapping {
        protected List<List<Room>> map = new ArrayList<List<Room>>(constraints.getMaxKeys());
    ...
    /**
     * Thrown by several IDungeonGenerator methods that can fail. Should be
     * caught and handled in {@link #generate}.
     */
     ...
     * @see #placeKeys
     */
    protected static final Comparator<Room> EDGE_COUNT_COMPARATOR = new Comparator<Room>() {
        // @Override
        ...
        // @Override
        public int compare(Room arg0, Room arg1) {
            return arg0.getIntensity() > arg1.getIntensity() ? -1 : arg0.getIntensity() < arg1
                    .getIntensity() ? 1 : 0;
        }
    };
     ...
     */
    protected void initEntranceRoom(KeyLevelRoomMapping levels) throws RetryException {
        Coords coords = null;
        List<Coords> possibleEntries = new ArrayList<Coords>(constraints.initialCoords());
        ...
        Room entry = new Room(coords, null, new Symbol(Symbol.START), new Condition());
        dungeon.add(entry);
    ...
    protected void placeRooms(KeyLevelRoomMapping levels) throws RetryException {
        ...
        final int roomsPerLock = constraints.getMaxSpaces() / constraints.getMaxKeys();
     ...
     * @see KeyLevelRoomMapping
     */
    protected void placeBossGoalRooms(KeyLevelRoomMapping levels) throws RetryException {
        ...
        for (Room room : dungeon.getRooms()) {
            ...
            if (parent == null || parent.getChildren().size() != 1 || room.getItem() != null
        ...
        Room goalRoom = possibleGoalRooms.get(random.nextInt(possibleGoalRooms.size())), bossRoom = goalRoom
                .getParent();

        goalRoom.setItem(new Symbol(Symbol.GOAL));
        bossRoom.setItem(new Symbol(Symbol.BOSS));

        int oldKeyLevel = bossRoom.getPrecond().getKeyLevel(), newKeyLevel = levels.keyCount();
     ...
     */
    protected boolean switchLockChildRooms(Room room, Condition.SwitchState givenState) {
        boolean anyLocks = false;
        Condition.SwitchState state = givenState != Condition.SwitchState.EITHER ? givenState
                : (random.nextInt(2) == 0 ? Condition.SwitchState.ON : Condition.SwitchState.OFF);
    ...
    protected void placeSwitches() throws RetryException {
        ...
        for (int attempt = 0; attempt < 10; ++attempt) {
            ...
            for (Room room : rooms) {
                if (room.getItem() == null && baseRoomCond.implies(room.getPrecond())) {
                    switchRoom = room;
                    break;
                }
            }
    ...
    protected void graphify() throws RetryException {
        for (Room room : dungeon.getRooms()) {
            ...
            for (Direction d : Direction.values()) {
                ...
                boolean forwardImplies = room.precond.implies(nextRoom.precond), backwardImplies = nextRoom.precond
                        .implies(room.precond);
                if (forwardImplies && backwardImplies) {
                    ...
                    Symbol difference = room.precond.singleSymbolDifference(nextRoom.precond);
                    if (difference == null
                            || (!difference.isSwitchState() && random.nextInt(5) != 0))
                        continue;
    ...
    protected double applyIntensity(Room room, double intensity) {
        intensity *= 1.0 - INTENSITY_GROWTH_JITTER / 2.0 + INTENSITY_GROWTH_JITTER
                * random.nextDouble();
     ...
     * @see Room
     */
    protected void computeIntensity(KeyLevelRoomMapping levels) throws RetryException {
        ...
        for (int level = 0; level < levels.keyCount(); ++level) {
            ...
            for (Room room : levels.getRooms(level)) {
                if (room.getParent() == null
                        || !room.getParent().getPrecond().implies(room.getPrecond())) {
    ...
    public void generate() {
        ...
        while (true) {
            try {
                ...
                // Maps keyLevel -> Rooms that were created when lockCount had
                // that
                // value
